...
//        }
        ...
        // 是否设置了启用连接池
        if(isset($dbconfig['db_poor'])){
            $boolPoor = (bool) $dbconfig['db_poor'];
        }

        $this->usePool = $boolPoor;
        ...
        if ($this->connection) {
            ...
            try {
                if ($this->usePool) {
                    ...
                    $this->connection = new \pdoProxy($dsn, $user, $password, $options);
                ...
                if (strstr($ex->getMessage(), "connect to pool_server fail")) {//端口连不上
                    $this->connection = new \PDO($dsn, $user, $password, $options);
                    $this->usePool = false;
                } elseif (strstr($ex->getMessage(), "Can't connect to MySQL server")) {
                    $Newex = new Exception($ex->getMessage() . '(ServerIp:' . $_SERVER['HOSTNAME'] . ')');
                    $this->throwException($Newex);
                } else {
